...
line3, = axes.plot(range(len(test_y_disorder)), test_y_disorder, 'g', label='shiji')
line1, = axes.plot(range(len(gbr_pridict_disorder)), gbr_pridict_disorder, 'b--', label='jichengmoxing', linewidth=2)
line2, = axes.plot(range(len(mlp_pridict_disorder)), mlp_pridict_disorder, 'r--', label='duocengganzhiqi', linewidth=2)
axes.grid()
fig.tight_layout()
